            ...
            <el-row :span="20">
                ...
                <el-col :span="7">
                    <!-- <el-form-item label="所属类目：" prop="" size="mini">
                        <el-input v-model="searchParams.categoryName"  placeholder=""></el-input>

                    </el-form-item> -->
                    <category-select required  v-on:selectResult="selectResult" ></category-select>
                </el-col>
                <el-col :span="7">
            ...
            <el-row :span="20">
                ...
                <el-col v-show="searchParams.isQuality == 1" :span="60">
                    <el-form-item  label="保质期天数(天)：" required prop=""  size="mini">
                        <el-input @change="qualityDayFilter(searchParams.qualityDay)" maxlength="16" :value="searchParams.qualityDay" v-model="searchParams.qualityDay" type="text" placeholder=""></el-input>
                    </el-form-item>
                </el-col>
            </el-row>
            <el-row :span="20">
         ...
         <el-form size="small" class="" label-width="145px">
                ...
                <el-form-item v-for="(data, index) in propertyOptions" required prop=""  :key="index" :label="data.name+'：'">
                    <el-select  v-model="data.value" filterable placeholder="请选择">
                        <el-option
                            ...
                            @click.native="activeSelectNature(item,index)"
                            :key="index1"
                            :label="item.label"
                            :value="item.value">
                        </el-option>
            ...
            <el-card>
                ...
                <el-form-item  v-for="(data, index) in checkedAttributes" :key="index"  :label="data.name+'：'">
                    <el-checkbox-group v-model="selectCheck" @change="handleChecked">
                        <el-checkbox @change="handleCheckedChild(childData, index)" v-for="(childData, index1) in data.list" :key="index1" :label="childData.value">
                            <el-input :maxlength="100" v-show="selectCheck.indexOf(childData.value) > -1"  @input="handleCheckedChild(childData, index)" :disabled="selectCheck.indexOf(childData.value) == -1"  style="width:80px;" v-model="childData.label" size="mini" >
                                <!-- {{ childData.label }} -->
                            </el-input>
            ...
            <el-card  style="margin:20px 0 20px 0;">
                <div slot="header">
                 ...
                 <el-table class="minegood_add" id="tableMain" :data="tableData" border style="width: 100%">
                     ...
                     <!-- <el-table-column align="center" prop="marketPrice2"  label="参考市场价(元)" >
                         <template slot-scope="scope">
                             <el-input maxlength="10" v-model="scope.row.marketPrice2" type="text" size="mini"></el-input>
                         </template>
                     </el-table-column> -->
                     ...
                     <el-table-column align="center" prop="picture"  label="图片(最多三张)" >
                         <template slot-scope="scope">
                             <el-upload
                                ...
                                <!-- <i class="el-icon-plus"></i> -->
                                <!-- <el-button size="mini"  type="primary">点击上传</el-button> -->
                                <!-- <div slot="tip" class="el-upload__tip">注：最多限传5张，单张限3MB以内，支持JPG,PNG,BMP格式</div> -->
                            </el-upload>
                            <ul>
                                 <li v-for="(data, index) in scope.row.pictureArr" :key="index">
                                     <img style="width:50px; padding:5px 10px 5px 0;" :src="data.url"/> <el-button  @click="deleteCurrentImg(scope.$index, data)" size="mini" type="text">删除</el-button>
                                 </li>
                             </ul>
                         </template>
                    </el-table-column>
  ...
  export default {
    ...
    methods: {
        ...
        handleCheckedChild(obj, index) {
            ...
            if (num === this.checkedAttributes.length) {
                ...
                for (let i = 0, len = result.length; i < len; i++) {
                    ...
                    for (let j = 0; j < this.tableData.length; j++) {
                        ...
                        if (resId === tableId ) {
                            ...
                            // this.tableData[j].flagId = flagId;
                            flag = false;
                            break;
                        }
                    }
        ...
        keepData() {
            ...
            let data = this.searchParams;
                if (this.fileListShow.length) data.mainPicture = this.fileListShow.join(',');
                // data.updateFlagForSale = 0;
            ...
            this.$AXIOS.POST('/goods/goods', data).then(res => {
        ...
        categoryListInit(id, arrId) {
            this.$AXIOS.GET('/category/propertyValues/multiIdsSearch', {propertyIds: id}).then(res => {
                res.result.map((item, index) => {
                    for (let i = 0; i < this.propertyOptions.length; i++) {
                        if (item.propertyId === this.propertyOptions[i].propertyId && item.isValid !== '0') {
