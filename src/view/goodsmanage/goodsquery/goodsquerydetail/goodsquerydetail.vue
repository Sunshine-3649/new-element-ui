            ...
            <el-row :span="20">
                ...
                <el-col :span="7">
                    <el-form-item label="所属品牌："  prop="" size="mini">
                        ...
                        <!-- <el-select disabled v-model="searchParams.brandId" filterable placeholder="请选择">
                            <el-option
            ...
            <el-row :span="20">
                <el-col :span="10">
                    <el-form-item label="商品主图："  size="mini">
                        <a v-for="(data, index) in reditMainPicture" :key="index" :href="data.url" target='_blank'>
                            <img style="width:60px;margin-right:10px;"  v-if="data.url.indexOf('http') < 0" :src="imgSourceUrl + data.url"/>
                            <img style="width:60px;margin-right:10px;"  v-else :src="data.url"/>
                        </a>
                    </el-form-item>
                </el-col>
            </el-row>
         ...
         <el-form size="small" class="" label-width="145px">
            <el-card style="margin:20px 0 20px 0;" class="box-card">
                ...
                <el-row :span="24">
                    ...
                    <el-col :span="8">
                        <el-form-item  label="商品状态："  prop=""  size="mini">
                            {{ tableData.isValid == 1 ? '启用' : '停用' }}
                        </el-form-item>
                    </el-col>
                    <el-col :span="8">
                ...
                <el-row :span="24">
                    <el-col :span="8">
                        <el-form-item  label="条形码："  prop=""  size="mini">
                            <span style=" word-break:break-all">{{ tableData.barCode }}</span>
                        </el-form-item>
                    </el-col>
                    <el-col :span="8">
                        <el-form-item  label="市场参考价(元)："  prop=""  size="mini">
                            {{ tableData.marketPrice2 }}
                        </el-form-item>
                    </el-col>
                    <el-col :span="8">
                        <el-form-item  label="商品重量(KG)："  prop=""  size="mini">
                            {{ tableData.weight2 }}
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :span="24">
                    <el-col :span="24">
                        <el-form-item  label="图片："  prop=""  size="mini">
                            <a v-for="(data, index) in tableData.pictureArr" :key="index" :href="data" target='_blank'>
                                <img style="width:60px;margin-right:10px;"  v-if="data.indexOf('http') < 0" :src="imgSourceUrl + data"/>
                                <img style="width:60px;margin-right:10px;"  v-else :src="data"/>
                            </a>
                        </el-form-item>
                    </el-col>
                </el-row>
            ...
            <el-card style="margin:20px 0 20px 0;" class="box-card">
                ...
                <el-form-item v-for="(data, index) in resultData.itemNatureProperys" :key="index" :label="data.propertyName+'：'">
                  ...
                  <!-- <span  v-if="data.isSelected == 1">{{ data.propertyValue }}</span> -->
                </el-form-item>
            </el-card>
            <el-card>
                <div slot="header">
                ...
                <span v-show=" specInfoArr.constructor == Array" style="padding-left:50px;"  v-for="(data) in specInfoArr" :key="data" >
                    <span style="padding-right:30px;">{{ data }}</span>
                </span>
                <span v-show=" specInfoArr.constructor !== Array"> {{ specInfoArr }}</span>
            </el-card>
            <!-- <el-card style="margin:20px 0 20px 0;">
                <div slot="header">
                    <span>仓库反馈库存信息</span>
                </div>
                <el-table show-summary :summary-method="getSummaries"  :data="messageData " border style="width: 100%">
                    ...
                    <el-table-column align="center" prop="defectiveInventory"  label="残品总库存" > </el-table-column>
                </el-table>
            </el-card> -->

         </el-form>
         <div style="text-align:center;">
             <el-button style="margin:10px 0" size="mini" @click="back" >{{ backLastRouter ? '关闭' : "返回"}}</el-button>
         </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    data() {
        return {
            ...
            goodSkucode: this.$route.params.skuCode, // 编辑商品skucode
    ...
    methods: {
        ...
        // 编辑初始化
        goodInitRedit() {
            this.$AXIOS.GET('/goods/productCenter/spuCode/' + this.goodSpucode, {skuCode: this.goodSkucode} ).then(res => {
                ...
                res.result.skus.map(item => {
                    ...
                    this.messageData = item.requestSkuStockList;

                    if (item.specInfo && item.specInfo.indexOf('|') > -1) {
                        this.specInfoArr = item.specInfo.split('|');
                    } else {
                        this.specInfoArr = item.specInfo;
                    }
                });
                console.log(this.specInfoArr);
        ...
        back() {
            if (this.backLastRouter) {
                ...
                this.$router.push({path: '/goodsquery'});
            }
        },
        // 查询分类关联品牌
        categoryList(id) {
