  ...
  <div class="transordermanage">
    ...
    <el-form :model="searchParams" ref="queryForm" label-width="125px" class="search_form" size="medium">
      <el-row :span="20">
        ...
        <el-col :span="6">
          <el-form-item label="出入库状态：" size="mini">
            <el-select v-model="searchParams.inOutStatus" filterable placeholder="请选择" @change='outInTypeChange'>
              <el-option v-for="item in inOutStatusList" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
      ...
      <el-row :span="20">
        <el-col :span="6">
          <el-form-item label="单据状态：" size="mini">
            <el-select v-model="searchParams.orderStatus" filterable placeholder="请选择">
        ...
        <el-col :span="6">
          <el-form-item label="最近更新时间：" size="mini">
            <el-col :span="11">
              <el-form-item size="mini">
                <el-date-picker v-model="searchParams.updateTimeStart" :clearable='false' type="date" placeholder="选择日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd" style="width: 100%">
                </el-date-picker>
              </el-form-item>
            </el-col>
            <el-col style="text-align: center" :span="2">-</el-col>
            <el-col :span="11">
              <el-form-item size="mini">
                <el-date-picker v-model="searchParams.updateTimeEnd" type="date" :clearable='false' placeholder="选择日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd" style="width: 100%">
                </el-date-picker>
              </el-form-item>
            </el-col>
          </el-form-item>
    ...
    <el-row type="flex" justify="space-between">
      <el-col>
        <el-button type="primary" size="mini" @click="toAddOrEdit('add1')">+新增调拨单</el-button>
        <el-button type="primary" size="mini" @click="exportTel" :loading="disabledDownLoad" :disabled='isTableDisabled'>{{disabledDownLoad?'下载中':'导出'}}</el-button>
      </el-col>
      <el-col class='paging'>
    ...
    <el-table :data="tableDataList" @row-click="tableRowClick" ref="multipleTable" highlight-current-row border tooltip-effect="dark" height="454">
      ...
      <el-table-column label="调拨单编号" prop="allocateOrderCode" align="center" width="125" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column label="创建人" prop="createOperatorName" align="center" show-overflow-tooltip></el-table-column>
      <el-table-column label="单据状态" prop="orderStatus" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          <div v-if="scope.row.orderStatus === '3'" style="color: orange">{{checkedStatus(scope.row.orderStatus)}}</div>
          <div v-else>{{checkedStatus(scope.row.orderStatus)}}</div>
        </template>
      </el-table-column>
      <el-table-column label="出入库状态" prop="inOutStatus" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          <!-- <div v-if="scope.row.orderStatus === '5'">-</div> -->
          <div >{{checkedType(scope.row.inOutStatus)}}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="220">
        <template slot-scope="scope">
          ...
          <div v-else>
            <div v-if="scope.row.orderStatus === '0' || scope.row.orderStatus === '3'">
              <el-button size="mini" type="text"  @click="toAddOrEdit(scope.row.allocateOrderCode)">编辑</el-button>
              <el-button size="mini" type="text"  @click="deleteBtn(scope.row)">删除</el-button>
            </div>
            <div v-else-if="scope.row.orderStatus === '2'">
              <el-button size="mini" type="text"  @click="warehouseNameNotice(scope.row)">通知仓库</el-button>
              <el-button size="mini" type="text"  @click="discardBtn(scope.row, '1')">作废</el-button>
            </div>
            <div v-else-if="scope.row.orderStatus === '4' && (scope.row.outOrderStatus === '0' || scope.row.outOrderStatus === '2' || scope.row.outOrderStatus === '5')">
              <el-button size="mini" type="text"  @click="discardBtn(scope.row, '2')">作废</el-button>
            </div>
            <div v-else>-</div>
          </div>

          <!-- <el-button size="mini" type="text" v-if="scope.row.orderStatus === '0' || scope.row.orderStatus === '3'" @click="toAddOrEdit(scope.row.allocateOrderCode)">编辑</el-button>
          <el-button size="mini" type="text" v-if="scope.row.orderStatus === '0' || scope.row.orderStatus === '3'" @click="deleteBtn(scope.row)">删除</el-button>
          <el-button size="mini" type="text" v-if="scope.row.orderStatus === '2'" @click="warehouseNameNotice(scope.row)">通知仓库</el-button>
          <el-button size="mini" type="text" v-if="scope.row.orderStatus === '2' || (scope.row.orderStatus === '4' && scope.row.outOrderStatus === '0')" @click="discardBtn(scope.row)">作废</el-button>
          <el-button size="mini" type="text" v-if="scope.row.orderStatus !== '0' &&  scope.row.orderStatus !== '2' && scope.row.orderStatus !== '3' && (scope.row.orderStatus !== '4'  || scope.row.outOrderStatus !== '0')" >-</el-button> -->
        </template>
      </el-table-column>
    </el-table>
    <!-- 选项卡 -->
      ...
      <el-tab-pane label="商品明细" name="1">
        ...
        <el-row>
          <el-col :span="6" style="height: auto;line-height: 30px;">
            <div class="title">调拨出库通知单编号：</div>
            <div class="content">{{tableRowDetail.allocateOutOrderCode}}</div>
          </el-col>
          <el-col :span="6" style="height: auto;line-height: 30px;">
            <div class="title">调拨入库通知单编号：</div>
            <div class="content">{{tableRowDetail.allocateInOrderCode}}</div>
          </el-col>
        </el-row>
        <el-table :data="skuDetailList" style="width: 100%" border>
          ...
          <el-table-column prop="inventoryType" label="调拨库存类型" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column prop="planAllocateNum" label="计划调拨数量" align="center">
            <template slot-scope="scope">
              <div v-if="scope.row.planAllocateNum !== null && scope.row.planAllocateNum !== ''">{{scope.row.planAllocateNum}}</div>
              <div v-else>-</div>
            </template>
          </el-table-column>
          <el-table-column prop="realOutNum" label="实际出库数量" align="center">
            <template slot-scope="scope">
              <div v-if="scope.row.realOutNum !== null && scope.row.realOutNum !== ''">{{scope.row.realOutNum}}</div>
              <div v-else>-</div>
            </template>
          </el-table-column>
          <el-table-column prop="allocateOutStatus" label="出库状态" align="center">
            <template slot-scope="scope">
              <div v-if="scope.row.allocateOutStatus === '3'" style="color: orange">{{checkAllocateOutStatus(scope.row.allocateOutStatus)}}</div>
              <div v-else>{{checkAllocateOutStatus(scope.row.allocateOutStatus)}}</div>
            </template>
          </el-table-column>
          <el-table-column label="仓库反馈入库信息" align="center" >
            <el-table-column prop="realInNum" label="实际入库总量" align="center">
              <template slot-scope="scope">
                <div v-if="scope.row.realInNum !== null && scope.row.realInNum !== ''">{{scope.row.realInNum}}</div>
                <div v-else>-</div>
              </template>
            </el-table-column>
            <el-table-column prop="nornalInNum" label="正品入库数量" align="center">
              <template slot-scope="scope">
                <div v-if="scope.row.nornalInNum !== null && scope.row.nornalInNum !== ''">{{scope.row.nornalInNum}}</div>
                <div v-else>-</div>
              </template>
            </el-table-column>
            <el-table-column prop="defectInNum" label="残品入库数量" align="center">
              <template slot-scope="scope">
                <div v-if="scope.row.defectInNum !== null && scope.row.defectInNum !== ''">{{scope.row.defectInNum}}</div>
                <div v-else>-</div>
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column prop="allocateInStatus" label="入库状态" align="center">
            <template slot-scope="scope">
              <div v-if="scope.row.allocateInStatus === '3'" style="color: orange">{{checkAllocateInStatus(scope.row.allocateInStatus)}}</div>
              <div v-else>{{checkAllocateInStatus(scope.row.allocateInStatus)}}</div>
            </template>
          </el-table-column>
        </el-table>
         <!-- 分页 -->
...
export default {
  ...
  data() {
    return {
      ...
      searchParams: {
        ...
        createTimeEnd: '', // 创建日期
        updateTimeStart: '', // 最近更新日期
        updateTimeEnd: '', // 最近更新日期
        start: 0, // 开始页面
        pageSize: 10, // 默认10条
        pageNo: 1 // 默认第一页
      }, // 请求参数
  ...
  created() {
    this.initDataList();
    this.getOrderStatusList(); // 获取单据状态列表
    this.getWarehouseList();  // 查询仓库列表
    this.getInOutStatusList();  // 查询出入库状态列表
  },
  methods: {
    exportTel() {
      let params = {};
        ...
        Object.keys(this.searchParams).forEach(key => {
          if (this.searchParams[key] !== '' && this.searchParams[key] !== undefined) {
            params[key] = this.searchParams[key];
          }
        });
        console.log(params);
        this.$AXIOS.EXPORT({url: '/allocateOrder/exportOrder', data: params, fileName: '调拨单'}).then(res => {
    ...
    // 获取单据状态列表
    getOrderStatusList() {
      this.$AXIOS.GET('/select/selectByTypeCode/allocateOrderStatus').then(res => {
        this.orderStatusList = [...this.orderStatusList, ...res.result];
      });
    },
    // 查询仓库列表
    getWarehouseList() {
    ...
    // 查询出入库状态列表
    getInOutStatusList() {
      this.$AXIOS.GET('/select/selectByTypeCode/allocateOrderInOutStatus').then(res => {
        this.inOutStatusList = [...this.inOutStatusList, ...res.result];
      });
    },
    // 库存类别翻译
    checkStockType(val) {
    ...
    // 出库状态翻译
    checkAllocateOutStatus(val) {
      return this.$utils.querydiction('allocateOrderSkuOutStatus', val);
    },
    // 入库状态翻译
    checkAllocateInStatus(val) {
      return this.$utils.querydiction('allocateOrderSkuInStatus', val);
    },
    outInTypeChange(val) {
    ...
    initDataList() {
      ...
      this.$AXIOS
        .LISTGET('/allocateOrder/page', this.searchParams)
        .then(res => {
          ...
          if (this.tableDataList && this.tableDataList.length) {
            setTimeout(() => {
              ...
              this.operateParams.entityId = this.tableDataList[0].allocateOrderCode;
              this.getLogInfo();
            }, 20);
          } else {
            this.tableRowList = [];
    ...
    // 获取采购单明细数据详情
    getWarehouseNoticeInfo(allocateOrderCode) {
      this.$AXIOS.GET('/allocateOrder/editGet/' + allocateOrderCode).then(res => {
        console.log(res);
    ...
    // 获取采购单明细数据详情
    getTableDetailList() {
      this.$AXIOS.LISTGET('/allocateOrder/page', this.detailParams).then(res => {
        console.log('res: ', res);
    ...
    deleteUpdateState(row) {
      this.$AXIOS
        .PUT('/allocateOrder/delete/' + row.allocateOrderCode)
        .then(res => {
    ...
    discardUpdateState(row) {
      this.$AXIOS
        .PUT('/allocateOrder/setDrop/' + row.allocateOrderCode)
        .then(res => {
          this.$utils.messagetip.successTip(res.databuffer);
          setTimeout(() => {
            this.initDataList();
    ...
    // 通知仓库的作废接口
    discardHouseUpdateState(row) {
      this.$AXIOS
        .PUT('/allocateOrder/drop/' + row.allocateOrderCode)
        .then(res => {
          this.$utils.messagetip.successTip(res.databuffer);
          setTimeout(() => {
            this.initDataList();
    ...
    warahouseAdvice(row) {
      this.$AXIOS
        .PUT('/allocateOrder/noticeWarehouse/' + row.allocateOrderCode)
        .then(res => {
          this.$utils.messagetip.successTip(res.databuffer);
          setTimeout(() => {
            this.initDataList();
    ...
    // 点击作废按钮 id=1,审核通过下的作废；id=2,通知仓库下的作废
    discardBtn(row, id) {
      this.$confirm(
        `确认后调拨单及其相关单据将一并被作废。`,
        `您确认要作废吗?`,
        ...
        .then(() => {
          if (id === '1') {
            ...
            this.discardHouseUpdateState(row);
          }
        })
        .catch(() => {
          this.$utils.messagetip.infoTip('已取消');
    ...
    warehouseNameNotice(row) {
      this.$confirm(`确认后将产生调拨出库通知单和调拨入库通知单。`, `您确认要通知仓库吗?`, {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
          center: true
    ...
    tableRowClick(row) {
      ...
      this.operateParams.entityId = row.allocateOrderCode;
      this.getLogInfo();
    },
    // 跳转新增页面
    toAddOrEdit(allocateOrderCode) {
      if (allocateOrderCode === 'add1') {
        this.$router.push({path: '/transordermanage/transorderaddoredit/' + allocateOrderCode, query: {isadd: 1}});
      } else {
        this.$router.push({path: '/transordermanage/transorderaddoredit/' + allocateOrderCode, query: {isadd: 0}});
      }
    },
    // 删除按钮
    ...
    searchFormData() {
      if (
        new Date(this.searchParams.createTimeStart).getTime() >
        new Date(this.searchParams.createTimeEnd).getTime()
      ) {
        this.$utils.messagetip.warningTip('创建时间的开始日期不得大于结束日期');
        return;
      }
      if (
        new Date(this.searchParams.updateTimeStart).getTime() >
        new Date(this.searchParams.updateTimeEnd).getTime()
      ) {
        this.$utils.messagetip.warningTip('最近更新时间的开始日期不得大于结束日期');
        return;
      }
    ...
    // 出入库状态翻译
    checkedType(status) {
      return this.$utils.querydiction('allocateOrderInOutStatus', status);
    },
    // 单据状态翻译
    checkedStatus(status) {
      return this.$utils.querydiction('allocateOrderStatus', status);
    },
    // 出库状态翻译
