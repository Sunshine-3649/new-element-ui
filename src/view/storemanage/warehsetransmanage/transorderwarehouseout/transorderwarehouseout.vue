<template>
  <div class="transorderwarehouseout">
    ...
    <el-form :model="searchParams" ref="queryForm" label-width="125px" class="search_form" size="medium">
      <el-row :span="20">
        <el-col :span="6">
          <el-form-item label="调拨出库单号：" size="mini">
            <el-input v-model.trim="searchParams.allocateOutOrderCode" placeholder=""></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
      ...
      <el-row :span="20">
        <el-col :span="6">
          <el-form-item label="出库单创建人：" size="mini">
    ...
    <el-table :data="tableDataList" ref="multipleTable" @row-click="tableRowClick" highlight-current-row border tooltip-effect="dark" height="454">
      ...
      <el-table-column label="调拨出库通知单编号" prop="allocateOutOrderCode" align="center" width='150'></el-table-column>
      ...
      <el-table-column label="出库单创建人" prop="createOperatorName" align="center" show-overflow-tooltip></el-table-column>
      <el-table-column label="状态" prop="status" align="center">
        <template slot-scope="scope">
          <el-tooltip v-if="scope.row.status === '2'" class="item" effect="dark" :content="scope.row.failedCause" placement="top">
            <div style="color: #CC6600">{{checkedStatusList(scope.row.status)}}</div>
          </el-tooltip>
          <el-tooltip v-else-if="scope.row.status === '4'" class="item" effect="dark" :content="scope.row.failedCause" placement="top">
        ...
        <template slot-scope="scope">
          ...
          <div v-else>
            <div v-if="scope.row.status === '0'">
              ...
              <el-button size="mini" type="text" @click="noticGood(scope.row)">通知出库</el-button>
            </div>
            <div v-else-if="scope.row.status === '1'">
              <el-button size="mini" type="text" @click="cancleDeliver(scope.row)">取消出库</el-button>
            </div>
            <div v-else-if="scope.row.status === '2'">
              <el-button size="mini" type="text" @click="closeGoodBtn(scope.row)">关闭</el-button>
              <el-button size="mini" type="text" @click="againDeliver(scope.row)">重新出库</el-button>
            </div>
            <div v-else-if="scope.row.status === '5'">
              <div v-if="scope.row.isTimeOut === '0' && scope.row.isClose === '1'">
                <el-button size="mini" type="text" @click="cancleClose(scope.row)">取消关闭</el-button>
              </div>
              <div v-else-if="scope.row.isTimeOut === '0' && scope.row.isCancel === '1'">
                <el-button size="mini" type="text" @click="againDeliver(scope.row)">重新出库</el-button>
              </div>
              <div v-else>-</div>
            </div>
          ...
          <!-- <el-button size="mini" type="text" v-if="scope.row.status === '0' || scope.row.status === '2'" @click="closeGoodBtn(scope.row)">关闭</el-button>
          ...
          <el-button size="mini" type="text" v-if="scope.row.status === '0'" @click="noticGood(scope.row)">通知出库</el-button>
          <el-button size="mini" type="text" v-else-if="scope.row.status === '1'" @click="cancleDeliver(scope.row)">取消出库</el-button>
          <el-button size="mini" type="text" v-else-if="scope.row.status === '2' || (scope.row.status === '5' && scope.row.isTimeOut === '0' && scope.row.isCancel === '1')" @click="againDeliver(scope.row)">重新出库</el-button>
          <div v-else>-</div> -->
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-tabs v-model="activeName" type="card" style="margin-top: 15px;">
      <el-tab-pane label="商品明细" name="1">
        <el-table :data="tableRowList" style="width: 100%">
          ...
          <el-table-column prop="outStatus" label="出库状态" align="center">
            <template slot-scope="scope">
              <!-- <el-tooltip class="item" effect="dark" :content="scope.row.failedCause" placement="top"> -->
              <div v-if="scope.row.outStatus === '4'" style="color: #CC6600">{{checkedStatusDetail(scope.row.outStatus)}}</div>
              <!-- </el-tooltip> -->
              <div v-else>{{checkedStatusDetail(scope.row.outStatus)}}</div>
            </template>
          </el-table-column>
        </el-table>
        <!-- 分页 -->
    ...
    <el-dialog title="确定关闭" :visible.sync="isClose" width="30%">
      <h2 style='line-height: 30px'>您确认要关闭此调拨出库通知单吗?</h2>
...
export default {
  ...
  methods: {
    ...
    // 出库单状态列表
    getStatusList() {
      this.$AXIOS.GET('/select/selectByTypeCode/allocateOutOrderStatus').then(res => {
    ...
    initDataList() {
      ...
      this.$AXIOS
        .LISTGET('/allocateOutOrder/allocateOutOrderPage', this.searchParams)
        .then(res => {
    ...
    putCancelClose(row) {
      this.$AXIOS
        .PUT('/allocateOutOrder/cancelClose/' + row.id, {flag: 1})
        .then(res => {
          if (res.appcode === '1') {
            this.initDataList();
    ...
    putReceiptAdviceInfo(row) {
      this.$AXIOS
        .PUT('/allocateOutOrder/allocateOrderOutNotice/' + row.id, {id: row.id})
        .then(res => {
          if (res.appcode === '1') {
            this.initDataList();
            this.$utils.messagetip.successTip(res.databuffer);
    ...
    // 取消/重新发货接口
    cancelGoods(params) {
      this.$AXIOS.PUT('/allocateOutOrder/allocateOrderOutCancel/' + this.id, params).then(res => {
        if (res.appcode === '1') {
    ...
    // 关闭发货接口
    closeGoods(params) {
      this.$AXIOS.PUT('/allocateOutOrder/close/' + this.id, params).then(res => {
        if (res.appcode === '1') {
          this.isClose = false;
          this.initDataList();
          this.$utils.messagetip.successTip(res.databuffer);
    ...
    // 获取入库通知单管理数据详情
    getWarehouseNoticeInfo(allocateOutOrderCode) {
      this.$AXIOS
        .GET('/allocateOrder/editGet/' + allocateOutOrderCode)
        .then(res => {
          console.log(res);
    ...
    // 获取采购单明细数据详情
    getTableDetailList() {
      this.$AXIOS.LISTGET('/allocateOutOrder/allocateOutOrderPage', this.detailParams).then(res => {
        console.log('res: ', res);
