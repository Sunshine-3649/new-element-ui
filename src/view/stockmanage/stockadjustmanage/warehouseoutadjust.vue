<template>
  <div class="warehouseinadjust">
    ...
    <el-form :model="searchParams" ref="queryForm" label-width="125px" class="search_form" size="medium">
      <el-row :span="20">
        <el-col :span="6">
          <el-form-item label="出库调整单编号：" size="mini">
            <el-input v-model.trim="searchParams.stockAdjustOutCode" placeholder=""></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
    ...
    <el-table :data="tableDataList" @row-click="tableRowClick" ref="multipleTable" highlight-current-row border tooltip-effect="dark" height="454">
      ...
      <el-table-column label="出库调整单编号" prop="stockAdjustOutCode" align="center" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column label="单据状态" prop="stockAdjustOutStatus" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          <div v-if="scope.row.stockAdjustOutStatus === 0" >出库完成</div>
          <div v-else>-</div>
        </template>
      </el-table-column>
    ...
    <el-tabs v-model="activeName" type="card" style="margin-top: 15px;" >
      <el-tab-pane label="商品明细" name="1">
        <el-table :data="tableRowList" style="width: 100%">
          ...
          <el-table-column prop="inOutInventoryTypeDesc" label="出库库存类型" align="center" show-overflow-tooltip></el-table-column>
          <el-table-column v-if="ifadjustInventoryObject" prop="adjustInventoryObjectDesc" label="出库库存对象（门店）" align="center"></el-table-column>
          <el-table-column prop="inOutNum" label="出库数量" align="center"></el-table-column>
          <el-table-column prop="finishTime" label="出库时间" align="center"></el-table-column>
        </el-table>
        <!-- 分页 -->
        <el-col class='paging' align="right">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        operateParams: {
          ...
          pageSize: 10,
          operateType: 0,
          entityType: 'StockAdjustOutOrder',
          entityId: ''
        }, // 日志
        tableData: [],
    ...
    methods: {
      // 获取采购单数据列表
      initDataList() {
        this.$AXIOS.LISTGET('/stockAdjustOut/page', this.searchParams).then(res => {
          ...
          if (this.tableDataList && this.tableDataList.length) {
            setTimeout(() => {
              ...
              this.operateParams.entityId = this.tableDataList[0].stockAdjustOutCode;
      ...
      // 获取采购单明细数据详情
      getTableDetailList() {
        this.$AXIOS.LISTGET('/stockAdjustOut/details/' + this.id, this.detailParams).then(res => {
          this.tableRowList = res.result;
          this.detailCount = res.totalCount;
          console.log(this.tableRowList[0].adjustInventoryObject > -1);
      ...
      tableRowClick(row) {
        ...
        this.getTableDetailList();
        this.operateParams.entityId = row.stockAdjustOutCode;
