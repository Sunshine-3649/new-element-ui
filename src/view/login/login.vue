    ...
    <div  class="login-con">
      ...
      <el-form :model="loginByPsw" status-icon :rules="rules" ref="loginByPsw">
        ...
        <el-form-item label="验证码" prop="phoneCode">
          <el-input placeholder="请输入短信验证码" v-model="loginByPsw.phoneCode" @keyup.enter.native="submitForm('loginByPsw')">
            <template slot="append">
              <el-button type="text" @click="checkPhone" :disabled="isSend">{{isSend ? time + '秒后重发' : '获取验证码'}}</el-button>
            </template>
          </el-input>
        </el-form-item>
        <el-form-item>
          <el-row :gutter="20">
            <el-col :span="12" :offset="2">
              <el-button type="primary" :loading="isSubmitting" @click.enter="submitForm('loginByPsw')">登录</el-button>
            </el-col>
            <el-col :span="8">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      checkPhone() {
        axios({
          ...
          if (res.data.body) {
            ...
            this.$utils.messagetip.errorTip('该手机号尚未注册！');
          }
        });
      },
      getCode() {
      ...
      submitForm(formName) {
          ...
          if (!this.loginByPsw.password) {
            this.$utils.messagetip.warningTip('密码不能为空！');
            return;
          }
          if (!this.loginByPsw.phoneCode) {
            this.$utils.messagetip.warningTip('验证码不能为空！');
            return;
          }
          let encrypt = new JSEncrypt.JSEncrypt();
              encrypt.setPublicKey('MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDN5xYYCRJJV7py74srvOs4ODfJJ2cftZY9eNep9qFls4D4v/duR0DZo95VK+5Yp0OekyLbaYi16obz4DBa85Ap/SRcSijWtWC0Z14hAfmpsR2qwve3Jtf72SouNvfXFhOnpcxhHRN9GeND6dlq0Qk40K61CF79wLQbVOEJRXfspwIDAQAB');
            ...
            data: data
              ...
              if (res.data.code === '200') {
                ...
                //   document.cookie = 'wmsToken=' + res.data.body.token + ';path=/';
                  ...
                  // this.$utils.messagetip.successTip('登录成功！')
                ...
                //   });
                document.cookie = 'token=' + res.data.body.token + ';path=/';
                window.localStorage.userPhone = res.data.body.ident;
                window.localStorage.userID = res.data.body.userId;
                this.$router.push({
                    path: '/SelectChannel'
                });
              } else {
                  this.$utils.messagetip.errorTip(res.data.message);
              }
          });
      },
      setWarehouse(code) {
          this.POST('/wmsUserInfo/setWarehouseCodeSession/' + code).then( res => {
              this.$utils.messagetip.successTip('设置仓库成功');
    ...
    created() {
        ...
        // console.log(this.$store.state.selectChannel.muneTabList);
    }
  };
