<template>
  <div class="otherwarehousein">
    ...
    <el-table :data="tableDataList" ref="multipleTable" @row-click="tableRowClick" highlight-current-row border tooltip-effect="dark" height="454">
      ...
      <el-table-column label="其他入库通知单编号" prop="outInNoticeCode" align="center" width='150'></el-table-column>
      ...
      <el-table-column label="入库原因" prop="outInCause" align="center" show-overflow-tooltip></el-table-column>
      <el-table-column label="入库单联系人" prop="outInContact" align="center" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <el-button size="mini" type="text" v-if="scope.row.status === 1 || scope.row.status === 3" @click="noticGood(scope.row)">通知入库</el-button>
          <el-button size="mini" type="text" v-else-if="scope.row.status === 2" @click="cancleDeliver(scope.row.outInNoticeCode)">取消入库</el-button>
          <el-button size="mini" type="text" v-else-if="scope.row.status === 6 && scope.row.isAllowNotice === '1'" @click="againDeliver(scope.row)">重新入库</el-button>
          <div v-else>-</div>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-tabs v-model="activeName" type="card" style="margin-top: 15px;">
      <el-tab-pane label="商品明细" name="1">
        <el-table :data="tableRowList" style="width: 100%">
          ...
          <el-table-column prop="planOutInNum" label="计划入库数量" align="center" show-overflow-tooltip></el-table-column>
          <el-table-column label="仓库反馈入库信息" align="center" width='140'>
            <el-table-column prop="realOutInNum" label="实际入库总量" align="center" width='140'></el-table-column>
            <el-table-column prop="nornalOutInNum" label="正品入库数量" align="center"></el-table-column>
            <el-table-column prop="defectOutInNum" label="残品入库数量" align="center"></el-table-column>
          </el-table-column>
          <el-table-column prop="outInStatus" label="入库状态" align="center">
            <template slot-scope="scope">
    ...
    <el-dialog title="取消入库" :visible.sync="dialogVisible" width="30%">
      <h2 style='line-height: 30px'>您确认要取消入库吗？</h2>
...
export default {
  ...
  created() {
    ...
    this.statusList = [...this.statusList, ...this.$utils.querydiction('otherInNoticeStatus')];
  },
  methods: {
    // 仓库列表
    getPurchaseType() {
    ...
    initDataList() {
      ...
      this.$AXIOS
        .LISTGET('/otherOutInNotice/otherInNoticePage', this.searchParams)
        .then(res => {
    ...
    putReceiptAdviceInfo(row) {
      this.$AXIOS
        .PUT('/otherOutInNotice/otherInNotice/' + row.outInNoticeCode)
        .then(res => {
          if (res.appcode === '1') {
            this.initDataList();
    ...
    // 取消/重新发货接口
    cancelGoods(params) {
      this.$AXIOS.PUT('/otherOutInNotice/otherInNoticeCancel/' + params.outInNoticeCode, params).then(res => {
        if (res.appcode === '1') {
    ...
    getWarehouseNoticeInfo(outInNoticeCode) {
      this.$AXIOS
        .GET('/otherOutInNotice/otherInNotice/' + outInNoticeCode)
        .then(res => {
          console.log(res);
    ...
    // 获取采购单明细数据详情
    getTableDetailList() {
      this.$AXIOS.LISTGET('/otherOutInNotice/otherInNoticePage', this.detailParams).then(res => {
        console.log('res: ', res);
    ...
    // 重新发货
    againDeliver(row) {
      this.$confirm(`确认后将通知仓库重新入库。`, `您确认要重新入库吗？`, {
    ...
    // 点击通知发货
    noticGood(row) {
      this.$confirm(`确认后将向仓库发送入库通知单。`, `您确认要通知入库吗?`, {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        center: true
    ...
    checkedStatusList(status) {
      return this.$utils.querydiction('otherInNoticeStatus', status);
    },
    // 状态翻译
