<template>
  ...
  <div class="wmsresourcemanage">
    ...
    <tree-table ref="recTree" :isShow='isShow' wms='1' :list.sync="treeDataSource" @addFunc="addFunc" @editFunc="editFunc" @handlerExpand="handlerExpand" @orderByFunc="orderByFunc"></tree-table>
    <!-- 类型的弹窗 -->
    <el-dialog :title="title" :visible.sync="isAddorEditShow" style="min-width: 800px;" :show-close='false' :close-on-click-modal='false' :close-on-press-escape='false'>
      <el-form :model="form" :rules="formRules" ref="form" style="width: 500px;">
        ...
        <el-form-item label="所属权限：" prop="belong" label-width="120px" size="mini">
          <el-radio v-model="form.belong" label="1">仓级权限</el-radio>
        </el-form-item>
    ...
    <el-dialog title="编辑" :visible.sync="isAddorEditShow1" style="min-width: 800px;" :show-close='false' :close-on-click-modal='false' :close-on-press-escape='false'>
      <el-form :model="form" :rules="formRules" ref="form1" style="width: 500px;">
        ...
        <el-form-item v-if="level === 3 && isEditShow === 2 && isTypeShow" label="操作类型：" prop="operationType" label-width="120px" size="mini">
...
export default {
  ...
  methods: {
    getInitDataList() {
      this.$AXIOS.LISTGET('/aclWmsUser/aclWmsTree', this.searchParams).then(res => {
        this.treeDataSource = res.result || [];
      });
    },
    // 新增资源
    addJurisdictionSave() {
      this.$AXIOS.POST('/aclWmsUser/aclWmsSave', this.form).then(res => {
        if (res.appcode === '1') {
    ...
    putAclWmsUpdate(params) {
      this.$AXIOS.PUT('/aclWmsUser/aclWmsUpdate/' + params.id, params).then(res => {
        this.$utils.messagetip.successTip(res.databuffer);
