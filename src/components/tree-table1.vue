      ...
      <div class="tree-body">
        <table v-if='treeDataSource.length>0'>
          <tbody>
            <tr>
              <td>
                <tree-item v-for="(model,i) in treeDataSource" :isSort='isSort' :key="'root_node_'+i" :root="0" :num="i" @addFunc="addFunc" @inputChange1="inputChange" @editFunc="editFunc" @toggleValid="toggleValid" @handlerExpand="handlerExpand" :nodes="treeDataSource.length" :trees.sync="treeDataSource" :model.sync="model">
                </tree-item>
              </td>
            </tr>
          </tbody>
...
export default {
  ...
  components: {
    treeItem: () => import('./tree-item1.vue')
  },
  mounted() {
    const vm = this;
    vm.$nextTick(() => {
